...
function InspectTab({
    ...
    const plotlyTraces = useMemo(() => {
        ...
        // 4. Best Fit Envelope
        if (pixelFit.best_fit && pixelFit.success !== false) {
            traces.push({
                x: fitX,
        ...
        // 5. Residuals (offset at bottom)
        if (pixelFit.residuals && pixelFit.success !== false) {
            const maxVal = Math.max(...pixelFit.corrected.map((p: any) => p.y));
            traces.push({
                x: fitX,
...
def buildHistogram(values: number[], nBins: number) {
...
with open(r"C:\\Users\\Rodrigo\\Downloads\\Depurado 2\\src\\features\\map-analyzer\\components\\fitting-results-panel.tsx", "w", encoding="utf-8") as f:
