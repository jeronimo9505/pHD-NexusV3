...
def apply_pipeline_to_file(request: PipelineApplyRequest):
    ...
    try:
        ...
        with h5py.File(target_path, "w") as f:
            ...
            ds_wn = grp.create_dataset("wavenumbers", data=x_new.astype(np.float32), compression="gzip", shuffle=True)
            ds_wn.attrs["units"] = "cm^-1"
            ds_wn.attrs["label"] = "Raman Shift (Processed)"
            
            ds_int = grp.create_dataset("intensities", data=y_new.astype(np.float32), compression="gzip", shuffle=True)
